var pedigreejs = (function (exports) {
    ...
    function addPbuttonEvents(opts) {
        ...
        $("#" + opts.btn_target).on("click", function (e) {
            ...
            if ($(e.target).hasClass('fa-undo')) {
                ...
                messages("Reset Pedigree", "Questo può provocare la perdita di alcuni dati. Sei sicuro?", reset, opts);
    ...
    function reset(opts) {
        ...
        if (selected.length > 0 && selected.val() === 'extended2') {
            ...
            opts.dataset = [{
                ...
                "top_level": true,
                "status": "0",
                // "display_name": "Nonno paterno"
            }, {
                "name": "MAk",
                "sex": "F",
                "top_level": true,
                "status": "0",
                // "display_name": "Nonna paterna"
            }, {
                "name": "zwB",
                "sex": "M",
                "top_level": true,
                "status": "0",
                // "display_name": "Nonno materno"
            }, {
                "name": "dOH",
                "sex": "F",
                "top_level": true,
                "status": "0",
                // "display_name": "Nonna materna"
            }, {
                "name": "MKg",
                "sex": "F",
                "mother": "MAk",
    ...
    **/
    ...
    function openEditDialog(opts, d) {
        ...
        // table += "<tr><td style='text-align:right'>Anno di nascita</td><td><input class='form-control' type='number' id='id_yob' min='1900' max='2050' name='yob' style='width:7em' value=" + (d.data.yob ? d.data.yob : "") + "></td></tr>";
        ...
        // table += '<tr><td colspan="2" id="id_death_cause">' + '<label class="radio-inline"><input type="radio" name="death_cause" value="cancer" ' + (d.data.death_cause === 'cancer' ? "checked" : "") + '>Per tumore</label>' + '<label class="radio-inline"><input type="radio" name="death_cause" value="related" ' + (d.data.death_cause  === 'related' ? "checked" : "") + '>Correlato al tumore</label>' + '<label class="radio-inline"><input type="radio" name="death_cause" value="non_cancer" ' + (d.data.death_cause  === 'non_cancer' ? "checked" : "") + '>Non correlato al tumore</label>' + '</td></tr>';
        $("#id_status input[value='" + d.data.status + "']").prop('checked', true);
        // $("#id_status input[value='" + d.data.death_cause + "']").prop('checked', true);

        // switches
        let switches = ["adopted_in", "adopted_out", "miscarriage", "stillbirth", "termination"];
        // let switches = ["adottato_nella_famiglia", "adottato_fuori_dalla_famiglia", "aborto_spontaneo", "nato_morto", "aborto_indotto"];
        ...
        for (let iswitch = 0; iswitch < switches.length; iswitch++) {
            ...
            // table += '<label class="radio-inline"><input type="radio" id="id_' + attr + '" name="' + attr + '" value="0" ' + (d.data[attr] ? "checked" : "") + '>&thinsp;' + capitaliseFirstLetter(attr.replace('_', ' ')) + '</label>';
        }
        table += '</td></tr>';
